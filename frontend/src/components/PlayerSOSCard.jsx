    ...
    <div className="chart-wrap" style={{ marginBottom: 20 }}>
      ...
      {expanded && (
        <div style={{ marginTop: 16 }}>
          ...
          {/* Fantasy playoff weeks 14-17 */}
          {(() => {
            const playoffGames = opponents.filter(o => o.week >= 14 && o.week <= 17)
            if (!playoffGames.length) return null
            const playoffHardest = [...playoffGames].sort((a, b) => (b.def_rank ?? 0) - (a.def_rank ?? 0))
            const playoffEasiest = [...playoffGames].sort((a, b) => (a.def_rank ?? 99) - (b.def_rank ?? 99))
            const gameByWeek = Object.fromEntries(playoffGames.map(o => [o.week, o]))
            return (
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>
                  🏆 Fantasy Playoffs — Weeks 14–17
                </div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                  {[14, 15, 16, 17].map(wk => {
                    const o = gameByWeek[wk]
                    if (!o) return (
                      <div key={wk} style={{ flex: 1, textAlign: 'center', background: 'var(--surface2)', borderRadius: 6, padding: '5px 4px', border: '1px solid #ffffff22', opacity: 0.5 }}>
                        <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 2 }}>Wk {wk}</div>
                        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)' }}>BYE</div>
                        <div style={{ fontSize: 9, color: 'var(--muted)', marginTop: 2 }}>—</div>
                      </div>
                    )
                    const c = rankColor(o.def_rank)
                    return (
                      <div key={wk} style={{ flex: 1, textAlign: 'center', background: 'var(--surface2)', borderRadius: 6, padding: '5px 4px', border: `1px solid ${c}44` }}>
                        <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 2 }}>Wk {wk}</div>
                        <div style={{ fontSize: 12, fontWeight: 700 }}>vs {o.opponent}</div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: c }}>#{o.def_rank}</div>
                        <div style={{ fontSize: 9, color: 'var(--muted)' }}>{rankLabel(o.def_rank)}</div>
                      </div>
                    )
                  })}
                </div>
                <div style={{ display: 'flex', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--red)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Toughest</div>
                    {playoffHardest.slice(0, 2).map(o => (
                      <div key={o.week} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 8px', background: 'var(--surface2)', borderRadius: 6, marginBottom: 3 }}>
                        <span style={{ fontSize: 11 }}>Wk {o.week} <b>{o.opponent}</b></span>
                        <span style={{ fontSize: 10, fontWeight: 700, color: rankColor(o.def_rank) }}>#{o.def_rank}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--green)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Easiest</div>
                    {playoffEasiest.slice(0, 2).map(o => (
                      <div key={o.week} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 8px', background: 'var(--surface2)', borderRadius: 6, marginBottom: 3 }}>
                        <span style={{ fontSize: 11 }}>Wk {o.week} <b>{o.opponent}</b></span>
                        <span style={{ fontSize: 10, fontWeight: 700, color: rankColor(o.def_rank) }}>#{o.def_rank}</span>
                      </div>
